/**
  ...
  * \version 1.0.0
  * \date    1.08.2018
  * \brief   Пример программы на работу для выполнения лабораторной работы № 1а.
...
/**
  ...
  */
int getline(char* buf, int buf_len)
{
    char c;
    int pos = 0;
...
int main(void)
{
    ...
    for (;;)
    {
        ...
        /* Прочитать строку из стандартного потока и поместить в origin_str.
           ...
           Для чтения рекомендуется использовать функцию getline:
           getline(origin_str, sizeof(origin_str), '\n');
        ...
        if (getline(origin_str, sizeof(origin_str)) == 0)
        {
        ...
        printf("Find:\n");
        if (getline(find_str, sizeof(find_str)) == 0)
        {
            printf("Incorrect input\n");
            continue;
        }
        ...
        printf("Replace with:\n");
        if (getline(replace_with_str, sizeof(replace_with_str)) == 0)
        {
            printf("Incorrect input\n");
            continue;
        }

...
/* Массив с отсчетами гармонического сигнала */
#include "sin.h"

/* Подключение внешних переменных */
/* Начальные и конечные индексы в массиве sin_arr */
extern volatile uint32_t a_idx;
extern volatile uint32_t b_idx;
...
void DMA_IRQHandler(void)
{
    ...
    /* Определение адреса первого отсчета в sin_arr по a_idx */
    DMA_PriCtrlStr.DMA_SourceBaseAddr =
        (uint32_t)sin_arr + a_idx * sizeof(sin_arr[0]);

    /* Определение количества пересылок из sin_arr по a_idx и b_idx */
    DMA_PriCtrlStr.DMA_CycleSize = b_idx - a_idx + 1;

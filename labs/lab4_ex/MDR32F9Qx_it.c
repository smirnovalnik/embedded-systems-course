...
/* Буферы АЦП */
extern int16_t adc_buf1[];
extern int16_t adc_buf2[];

/* Флаги готовности буферов */
extern volatile uint8_t adc_buf1_rdy;
extern volatile uint8_t adc_buf2_rdy;
...
void DMA_IRQHandler(void)
{
    ...
    if (buf_num == 0)
    {
        /* Буфер 1 готов */
        adc_buf1_rdy = 1;
        /* На следующем прерывании используется другой буфер */
        buf_num ^= 0x01;

        /* Перенастройка канала DMA на adc_buf2 */
        DMA_PriCtrlStr.DMA_DestBaseAddr = (uint32_t)adc_buf2;
    ...
    else
    {
        ...
        /* Перенастройка канала DMA на adc_buf1 */
        DMA_PriCtrlStr.DMA_DestBaseAddr = (uint32_t)adc_buf1;
        DMA_Init(DMA_Channel_ADC1, &DMA_InitStr);
    }
}
/*******************************************************************************
